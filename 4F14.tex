...
\begin{itemize}
    \item \textit{Byte-addressable:} Each address referes to an 8-bit quantity (a byte).
...
\begin{itemize}
    ...
    \item Each track is divided into $\sim$ 850-2600 \textit{sectors}, which are the smallest unites that can be read/written.
...
\begin{itemize}
    \item \textit{Stack ISA}: Operands are taken from the stack.
    \item \textit{Accumulator ISA:} One operand is taken from the accumulator.
...
\begin{itemize}
    ...
    \item Jump and link: \texttt{jal A} means call procedure A and save the return address (PC + 4) in register \$31. If a procedure calls another procedure, the return address must be stored on the stack (fills top-down in MIPS), where the address of the bottom of the stack is in register \$29.
\end{itemize}

\textbf{Parameter passing:} The first 4 parameters are passed in registers \$4 to \$7, and the rest are passed on the stack. If another procedure is called, or registers are used as scratch space, registers need to be saved and restored, with two conventions:
\begin{itemize}
...
\section{ALU Design}
\textbf{Full adder:} Simpliefied boolean expression for the Sum and CarryOut:
\begin{align*}
...
\begin{center}
    ...
    \includegraphics[width=0.4\textwidth]{figures/4F14/32-bit_ALU.png}
\end{center}

\textbf{Ripple-carry 32-bit ALU:} Connect 32 1-bit ALUs, propagating the carry from one bit to the next. \texttt{slt} is performed by subtracting b from a and passing the Less signals to the Result line (Result 0 = 1 if a < b).
...
\begin{itemize}
    \item[] C1 = G0 + P0.c0
    \item[] C2 = G1 + P1.c4 = G1 + P1.G0 + P1.P0.c0
    \item[] C3 = G2 + P2.c8 = G2 + P2.G1 + P2.P1.G0 + P2.P1.P0.c0
    \item[] C4 = G3 + P3.c12 = G3 + P3.G2 + P3.P2.G1 + P3.P2.P1.G0 + P3.P2.P1.P0.c0
\end{itemize}
...
\textbf{Datapath:}
\begin{center}
    \includegraphics[width=0.7\textwidth]{figures/4F14/full_datapath.png}
\end{center}
...
\begin{center}
    \includegraphics[width=0.5\textwidth]{figures/4F14/datapath_control.png}
    \includegraphics[width=0.45\textwidth]{figures/4F14/ALU_control.png}
\end{center}
...
\begin{center}
    \includegraphics[width=0.7\textwidth]{figures/4F14/full_pipeline.png}
\end{center}
...
\begin{itemize}
    ...
    \begin{center}
        \includegraphics[width=0.7\textwidth]{figures/4F14/data_forwarding.png}
    \end{center}
...
\begin{itemize}
    ...
    \item Delayed branches: The instruction following the branch instruction is always executed (independent of the branch outcome).
\end{itemize}
...
\end{itemize}
\begin{center}
    \includegraphics[width=0.6\textwidth]{figures/4F14/set_associative.png}
\end{center}
...
\textbf{Virtual memory:} Virtual addresses are used and translated into physical address by the operating system.
...
\textbf{Translation lookaside buffer (TLB):} A special, fast cache to store recently used translations. The TLB only caches part of the page table that point to main memory (not disk), often fully associative with random placement. 
...
\begin{itemize}
    \item \textit{Memory-mapped I/O:} Each device appears as a set of data and control registers at a particular address. The control register contrains status bits that can be continully cheked (\textit{polling}).
    \item \textit{Interrupt-driven I/O:} An I/O device generates an interrupt when it requires attention. The CPU:
    \begin{enumerate}
...
\begin{itemize}
    \item If the DMA places data directly from disk into memory, the cache may contrain stale data.
    \item If the cache is write-back, the DMA unit might transfer stale data from memory to disk, by passing the newer data in the cache.
\end{itemize}
...
\textbf{Point-to-point networks:} Use a serial switched point-to-point network between devices to bypass the limit on bus length.
...
SIMD machines works best on arrays and vectors.
...
\textbf{Snooping:} All cache controllers monitor the bus and react to reads and writes in other caches to ensure cache coherence. Two ways to deal with writes:
\begin{itemize}
...
\textbf{MIMD networks:} Networks allow more processors than a single bus, where each processor has a local memory (\textit{distributed memory}).
Fully connected network has link between every pair of nodes but is expensive.

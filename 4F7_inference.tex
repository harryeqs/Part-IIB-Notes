...
\dot{x}(t) = A x(t) + MW(t)
$$
where $W(t)$ is a continuous-time white noise process with covariance with zero mean and autocorrelation function $R_{WW}(\tau) = \sigma_W^2\delta(\tau)$. The power of $W(t)$ is \textit{infinite}, therefore it is often define in terms of its integral \textit{Brownian motion} $B(t) = \int_{0}^{t}W(\tau)d\tau$.
The Brownian motion follows $B(t) \sim \mathcal{N}(0, t \sigma_W^2)$.
...
$$
x(t) = \exp (At) x(0) + \underbrace{\int_0^t \exp({A(t-\tau)}) M W(\tau) d\tau}_{= C, \text{ Convolution with white noise}}
$$
where $\exp(At) = I + At + A^2t^2/2! + A^3t^3/3! + \dots$
...
In discrete form
$$
x_{n+1} = \exp(A \delta_t)x_n + C, C \sim \mathcal{N}(0, \text{cov}(C))
$$
...
The Euler approximation is $x(t + \delta t) \sim \mathcal{N}(x(t)+\rho x(t)\delta_t, b^2 \delta_t \sigma_W^2)$ = first order expansion.
...
\end{align*}
where $v_t$ and $w_t$ are independent Gaussian noises with zero mean and covariance $\Sigma_v$ and $\Sigma_w$ respectively. The initial state is also Gaussian distributed:
...
\end{align*}
\begin{itemize}
    \item Kalman Gain $K_t$: How much the prediction should be adjusted based on the new observation.
    \item Innovation $y_t - B \mu_{t | t-1}$: How different is the observation from the prediction.
\end{itemize}
When the state space model is linear and Gaussian ($v_t, w_t, x_0$ are Gaussian distributed), Kalman filter is the \textit{optimal Bayesian estimator} of the state.

More generally, Kalman filter is the \textit{best linear, unbiased estimate (BLUE)} in an \textit{minimum mean square error (MMSE)} sense of the state given that $v_t, w_t$ are \textit{zero mean and uncorrelated}.
...
Usually, the linearisation points are $x^* = \mu_{t-1|t-1}$ and $x^+ = \mu_{t|t-1}$.
...
$$
\bar{h} = \int h(x) p(x) dx \approx \int h(x)  \frac{1}{N} \sum_{i=1}^N \delta_{x^{(i)}}(x) dx \approx \frac{1}{N} \sum_{i=1}^N h(x^{(i)})
$$
...
$$
\bar{h} = \int h(x_t) p(x_t | y_{1:t}) dx_t \approx \frac{1}{N} \sum_{i=1}^N w_t^{(i)} h(x_t^{(i)}), \quad x_t^{(i)} \overset{\text{i.i.d.}}{\sim} q(x_t)
$$
...
The normalisation constant is a Monte Carlo estimate $\hat{Z} = \sum_{i=1}^{N}\tilde{w}^{(i)}$ and $w^{(i)} = \frac{\tilde{w}^{(i)}}{\hat{Z}}$.
...
Samples can be drawn from the Monte Carlo approximation to $p(x_{t+1} | y_{1:t+1})$ by:
\begin{itemize}
    \item rejection sampling (slow but exact)
    \item importance sampling (most common)
    \item MCMC (effective but slow)
    \item annealed importance sampling etc.
\end{itemize}

\subsection*{General Sequential Importance Sampling}
Generate a new state from a specially designed proposal distribution $q(x_t,x_{t+1} | y_{1:t+1})$ rather than the state transition density $f(x_{t+1} | x_t)$:
\begin{align*}
...
Otherwise, the weights are reset to $1/N$ after resampling.
...
\begin{algorithm}
\caption{General Particle Filter}
\begin{algorithmic}
\For{$t = 1, \dots, T$}
    \For{$i = 1, \dots, N$}
        \State $x_{t+1}^{(i)} \sim q(x_{t+1}^{(i)} | x_t^{(i)}, y_{1:t+1})$
        \State $w_{t+1}^{(i)} \propto w_t^{(i)} \frac{g(y_{t+1} | x_{t+1}^{(i)}) f(x_{t+1}^{(i)} | x_t^{(i)})}{q(x_{t+1}^{(i)} | x_t^{(i)}, y_{1:t+1})}$
    \EndFor
    \State \textbf{(Optional Resample):}
    \State Normalise weights, $\sum_i w_{t+1}^{(i)} = 1$
    \For{$i = 1, \dots, N$}
        \State Set ${x'_{t+1}}^{(i)} = x_{t+1}^{(j)}$ with probability $w_{t+1}^{(j)}$
    \EndFor
    \For{$i = 1, \dots, N$}
        \State Set $x_{t+1}^{(i)} = {x'_{t+1}}^{(i)}$ and $w_{t+1}^{(i)} = 1/N$
    \EndFor
\EndFor
\end{algorithmic}
\end{algorithm}

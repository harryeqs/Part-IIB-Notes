...
$$
Define $S_N = \sum_{n=1}^N \tilde{f}(x_n)$ and $Z_N = \frac{S_N}{\sqrt{N}}$.
...
which is the MGF of the standard normal. Hence $Z_N \to \mathcal{N}(0, 1)$, $S_N \to \mathcal{N}(0, N)$, $\frac{1}{N} \sum_{n=1}^N f(x_n) \to \mathcal{N}(\mathbb{E}[f(x)], \frac{\sigma_f^2}{N})$.
...
The biggest decrease in variance is achieved when $q(x) \propto |f(x)| p(x)$, leading to $\sigma_{\bar{f}}^2 = 0$.
...
\begin{itemize}
    ...
    \item \textit{Uniform convergence:} Given $\epsilon > 0$, there exists a natural number $N(\epsilon)$ such that for $n > N$, $|f_n - f| < \epsilon, \forall x \in D$. Preserves continuity and \textit{allows exchange of limit and integral.}
\end{itemize}
...
\begin{itemize}
    ...
    \item A Riemann integrable function is also Lebesgue integrable, but not vice versa.
\end{itemize}
...
\begin{itemize}
    ...
    \item $\mu(\emptyset) = 0$.
    \item If $E_i \subseteq E_j$, then $\mu(E_i) \leq \mu(E_j)$. (Monotonicity)
    \item $\mu \left(\bigcup_{i=1}^\infty E_i\right) = \sum_{i=1}^\infty \mu(E_i)$ for disjoint sets $E_i \in \Sigma$. (Countable additivity)
...
If $p(x,y)$ satisfies detailed balance $\pi(x)p(x,y) = \pi(y)p(y,x)$, then $\pi(\cdot)$ is the invariant density of the transition kernel $P(x, \cdot)$. In this case the transition kernel is reversible, i.e. $P(x, dy) \pi(dx) = P(y, dx) \pi(dy)$.
...
\textbf{Gibbs sampler:} set kernels to $P_1(x_1, dy_1 | x_2) = \pi^*_{1|2}(dy_1 | x_2)$ and $P_2(x_2, dy_2 | x_1) = \pi^*_{2|1}(dy_2 | x_1)$. Use exact conditional densities as the proposal densities, i.e. for transition of $x_1$ with $x_2$ fixed, $q([x_1, x_2], [y_1, y_2]) = \pi(y_1 | x_2)$ and $q([y_1, y_2],[x_1, x_2]) = \pi(x_1 | y_2)$. As a result, the acceptance probability is $\alpha = 1$ for all moves.
...
The norm of a Hilbert space is induced by the inner product $\|f\| = \sqrt{\langle f, f \rangle}$. The $L^2$ norm $\|f - g\|_2$ measures the closeness of functions in the mean but not necessarily pointwise. When $\|f - g\|_2 = 0$, there is \textit{equivalence almost everywhere} except for sets of measure zero. Hence, the $L^2$ space is a space of \textit{equivalence classes of functions}.
...
\textbf{Function approximation by polynomial expansion:} By Fourier expansion, functions from the set $C_2[-\pi, \pi] = C[-\pi, \pi] \cap L^2[-\pi, \pi]$ can be represented as $f(x) = \sum_{k=0}^\infty c_k \exp(ikx)$ with $c_k = \frac{1}{2\pi} \int_{-\pi}^{\pi} f(x) \exp(-ikx) dx$. The $L^2$ norm of $f$ is $\|f\|^2 = \sum_{k=0}^\infty c_k^2$.
...
&< \frac{1}{n^{2s}} \sum_{k=n+1}^\infty k^{2s} c_k^2 <  \frac{1}{n^{2s}} \sum_{k=1}^\infty k^{2s} c_k^2 \\
...
\textbf{Bayes' rule in Hilbert space}: The Radon-Nikodym derivative of the posterior probability measure with respect to the prior probability measure is given by
$$
...
For $v = u + \beta \xi, \xi \sim \mathcal{N}(0, C)$ the acceptance probability is $\min(1, \exp(J(v) - J(u)))$, where $J(v) = - \Phi(v) - \frac{1}{2}|C^{-1/2} v|^2$. The expectation of the norm $\mathbb{E}[|C^{-1/2} v|^2]$ is infinite.  

\section{Langevin Dynamics}
\textbf{Langevin stochastic differential equation (SDE):} $$dX_t = -\nabla_x U(X_t) dt + \sqrt{2} dB_t = \nabla_x \log \pi(X_t) dt + \sqrt{2} dB_t$$
where $U(\cdot): \mathbb{R}^d \to \mathbb{R}$ is a potential function, $(B_t)_{t \geq 0}$ is the Brownian motion and $\pi(x) \propto \exp(-U(x))$ is the stationary distribution of the SDE.
...
For one dimension
$$
\nabla^2 \log \pi(x) \leq -m < 0 \; \forall x
$$
...
\textbf{Energy-based models:} The probability of a state $x$ is defined as 
...
EBMs parameterise the energy function, giving $p_\theta(x) \propto \exp(-E_\theta(x))$.

...
\textbf{Camera calibration:} The process of determining the projection matrix $\mathrm{P}$. Each point $(u_i, v_i)$ gives 2 equations, so at least 6 points are needed to calibrate the camera.
...
\begin{itemize}
    ...
    $$
    \lambda_1 \leq \frac{\mathbf{p}^\top \mathrm{A}^\top \mathrm{A} \mathbf{p}}{\mathbf{p}^\top \mathbf{p}} \leqq \lambda_{12}
    $$
\end{itemize}
...
$$
A minimum of 4 image correspondences can be used to estimate the homography and stitch images together.
...
\begin{enumerate}
    ...
    \item Repeat steps 2-4 tp maximise the number of inliers
\end{enumerate}

\textbf{Weak perspective projection:} When the depth of objects in the scene is small compared to their distance from the camera, the projection can be approximated as a parallel projection, with a weak perspective projection matrix

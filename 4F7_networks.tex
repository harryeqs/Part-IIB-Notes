...
\textbf{Bipartite graph:} a graph where the nodes can be split into 2 groups, such that all edges fall between groups and none within groups.
...
$$x_i = \alpha \sum_{j=0}^{n-1} A_{ij} x_j$$ For some constant $\alpha$. In matrix form, $\boldsymbol{x} = \alpha A \boldsymbol{x}$, which means $\boldsymbol{x}$ is an eigenvector of $A$. If $x_i \geq 0$, the solution is unique and will correspond to the eigenvector of $A$ with the largest eigenvalue.
...
If $x_i = x_j$ whenever $A_{ij} = 1$, then $r = 1$ and corresponds to \textit{perfect assortativity}. 
If $x_i = -x_j$ whenever $A_{ij} = 1$, then $r = -1$ and corresponds to \textit{perfect disassortativity}.

\textbf{Modularity:} generalises assortativity for categories, beyond numerical quantities.
...
\begin{align*}
\boldsymbol{y} &= \boldsymbol{b} + W^{(u)} \boldsymbol{y} \\
\boldsymbol{y} &= \left(I - W^{(u)}\right)^{-1} \boldsymbol{b}
\end{align*}
...
$$
(\mathcal{L} \boldsymbol{y})_i = \sum_j A_{ij} (y_i - y_j) = k_i y_i - \sum_j A_{ij} y_j
$$
The Laplacian matrix is $\mathcal{L} = D - A$, where $D$ is the diagonal degree matrix with $D_{ii} = k_i$ and $D_{ij} = 0$ for $i \neq j$.
The corresponding eigenvalue equation is $\mathcal{L} \boldsymbol{v} = \lambda \boldsymbol{v}$.

The signals can be written as a series in the eigenvectors of $\mathcal{L}$:
$$\boldsymbol{y} = \sum_{\alpha=0}^{n-1} c_\alpha \boldsymbol{v}_\alpha \text{ with } c_\alpha = \boldsymbol{v}_\alpha^\top \boldsymbol{y}$$

\textit{Facts:}
\begin{enumerate}
    \item The Laplacian matrix $\mathcal{L}$ is positive semi-definite, i.e. $\lambda_i \geq 0$ for all $i$.
    \item The constant vector $\boldsymbol{v}_0 = (1, 1, \ldots, 1)^\top$ is an eigenvector of $\mathcal{L}$ with eigenvalue $\lambda_0 = 0$.
...
\begin{align*}
\mathcal{L} &= D - A \\
\mathcal{L}^{\text{(rw)}} &= D^{-1} \mathcal{L} \text{ (average the differences)} \\
\mathcal{L}^{\text{(sn)}} &= D^{-\frac{1}{2}} \mathcal{L} D^{-\frac{1}{2}} \text{ (symmetric)}
\end{align*}

\subsection{Clustering}
Find an $\boldsymbol{x}$ that minimises
$$\sum_{i,j} A_{ij} (x_i - x_j)^2 = \boldsymbol{x}^\top \mathcal{L} \boldsymbol{x}$$
...
This yields $\boldsymbol{x}^\top \mathcal{L} \boldsymbol{x} = \sum_{j=1}^{n-1} c_j^2 \lambda_j$, which is minimised when $\boldsymbol{x} = \boldsymbol{v}_1$, the second eigenvector of $\mathcal{L}$.
...
\textbf{Giant component:} If $p$ were close to $1$, as $n \to \infty$, the size of the largest component would go to infinity -- the giant component.
...
\textbf{Squares:} In a Poisson random graph, the total number of cycles of length 4 is
$$
...
\textbf{The small components:} any node not in the unique giant component must be in a small component. The small components are distributed as 
$$
...
\textit{Fact:} For any fixed $p > 0$, the Watts-Strogatz model generates the small-world property, i.e. the distance between nodes is $\mathcal{O}(\log n)$.
...
\textit{Fact:} The preferential model has a power-law degree distribution $p_k \sim k^{-3}$.

Note:
\begin{itemize}
    \item The distribution is heavy-tailed
    \item Mean degree $\sum_k k p_k = 2c$
    \item the variance of degree distribution is $\sum_k (k - 2c)^2 p_k = \infty$ as $n \to \infty$
\end{itemize}
...
\textit{Fact:} A random network with degree distribution $p_k$ will have a giant component if $\mathbb{E}[k^2] > 2\mathbb{E}[k]$.
...
$$
P(\boldsymbol{g} | A, \omega) = \frac{P(A | \boldsymbol{g}, \omega) P(\boldsymbol{g})}{P(A | \omega)} = \frac{1}{Z}\prod_{i < j} \omega_{g_i g_j}^{A_{ij}} (1 - \omega_{g_i g_j})^{1 - A_{ij}}
$$
